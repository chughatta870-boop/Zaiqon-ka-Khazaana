...
const DATA_CATS = ["pakora","samosa","chips","kabab","roll","burger","chaat","pizza","shawarma","rice"];

async function loadData(){
  try{
    // fetch each small category file in parallel and merge —
    // avoids ever needing one giant data.json (which mobile
    // copy-paste uploads were truncating around ~20,000 chars)
    const results = await Promise.all(
      DATA_CATS.map(async (cat) => {
        const res = await fetch(`data-${cat}.json?v=3`, { cache: "no-store" });
        if (!res.ok) throw new Error(`data-${cat}.json → HTTP ${res.status}`);
        return res.json();
      })
    );
